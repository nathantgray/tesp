...
RUN echo "Cloning or download all relevant repositories..." && \
  ...
  cp -r tesp/src ${TESPDIR} && \
  cp -r tesp/data ${TESPDIR} && \
  cp tesp/DISCLAIMER.txt ${TESPDIR} && \
  cp tesp/LICENSE ${TESPDIR} && \
  cp tesp/README.md ${TESPDIR} && \
  cp tesp/requirements.txt ${TESPDIR} && \
  rm -r tesp && \
  echo "++++++++++++++ PSST" && \
